...
- [route-tests-and-realtime](route-tests-and-realtime.md) — route handlers made testable with a one-module mock (65 tests over the four assignment routes), the first `supabase_realtime` publication entry with a cross-ward leak test gating it, scenarios 012/013 trimmed to what needs a human; corrects six retros' "there is no local server", a timeout-based privacy test that passed while realtime was dead, and a shared-channel-topic bug in `CommentThread` that the missing publication had been masking (bf22ba2)
